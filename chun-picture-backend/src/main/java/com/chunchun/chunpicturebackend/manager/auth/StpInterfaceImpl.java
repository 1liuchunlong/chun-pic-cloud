...
import static com.chunchun.chunpicturebackend.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        ...
        // 如果所有字段都为空， 表示查询公共图库，仅代表通过权限校验 * 由于信息不足，先默认放行
        ...
        // 优先 -- 从上下文中获取 Spaceser 对象
        SpaceUser spaceUser = authContext.getSpaceUser();
        if (spaceUser != null) {
            return spaceUserAuthManager.getPermissionsByRole(spaceUser.getSpaceRole());
        }
        // 其次 --如果有 spaceUserId 必然是团队空间 通过数据库查询 SpaceUser 对象
        ...
        // -- 如果没有spaceUserId 尝试通过spaceId 或者 pictureId 获取Space 对象处理
        Long spaceId = authContext.getSpaceId();
        if (spaceId == null) {
            // --- 如果没有spaceId 通过 pictureId 获取Picture 对象和Space对象
            Long pictureId = authContext.getPictureId();
            // --- 如果pictureId也没有则默认通过权限校验
            if (pictureId == null) {
                return ADMIN_PERMISSIONS;
            }

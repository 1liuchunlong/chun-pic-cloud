...
import com.chunchun.chunpicturebackend.model.enums.SpaceRoleEnum;
import com.chunchun.chunpicturebackend.model.enums.SpaceTypeEnum;
 ...
 * @createDate 2025-12-26 11:55:12
 */
@Service
public class SpaceServiceImpl extends ServiceImpl<SpaceMapper, Space>
    ...
    public long addSpace(SpaceAddRequest spaceAddRequest, User loginUser) {
        ...
        //intern()的作用：它会去字符串常量池中查找。如果池中已经有了这个用户 ID 的字符串，就返回那个引用。
        synchronized (lock) {
            Long newSpaceId = transactionTemplate.execute(status -> {
                 ...
                 * eg: 已有私有空间不能再创建私有空间
                 * */
                boolean exists = this.lambdaQuery().eq(Space::getUserId,
                        userId).eq(Space::getSpaceType, space.getSpaceType()).exists();
                ThrowUtils.throwIf(exists, ErrorCode.OPERATION_ERROR,
                        "每个用户每类空间仅能创建一个");
                // 写入数据库
                boolean result = this.save(space);
                ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR);
                // 如果是团队空间 关联新增团队成员记录 把创建者添加为管理员
    ...
    public void validSpace(Space space, boolean add) {
        ...
        // 修改数据时，如果要改空间类型  应该是用不上
        // 传了一个不存在的空间类型  eg value不为0， 不为1
    ...
    public Page<SpaceVO> getSpaceVOPage(Page<Space> spacePage,
        ...
        // 1,2,3,4
        Set<Long> userIdSet =
                spaceList.stream().map(Space::getUserId).collect(Collectors.toSet());
        // 1 => user1, 2 => user2
    ...
    public QueryWrapper<Space> getQueryWrapper(SpaceQueryRequest spaceQueryRequest) {
        ...
        //isNotEmpty null 和 "" 都是false
        queryWrapper.eq(ObjUtil.isNotEmpty(id), "id", id);
        queryWrapper.eq(ObjUtil.isNotEmpty(userId), "userId", userId);
        queryWrapper.like(StrUtil.isNotBlank(spaceName), "spaceName",
                spaceName);
        queryWrapper.eq(ObjUtil.isNotEmpty(spaceLevel), "spaceLevel",
                spaceLevel);
